...
# ------------------------------------------------------------
# TEST API (matches unit tests)
# NOTE: tests call validate_basic_schema(obj, max_recs=...)
# ------------------------------------------------------------
def validate_basic_schema(obj: Dict[str, Any], max_recs: int = 12, **kwargs) -> List[str]:
    ...
    if "recommendations" in obj and "warnings" in obj:
        if not isinstance(obj["recommendations"], list):
            ...
            if len(obj["recommendations"]) > max_recs:
...
def validate_content_rules(obj: Dict[str, Any], **kwargs) -> List[str]:
    ...
    Old schema: checks banned phrases in joined recommendations/warnings.
    New schema: checks banned phrases + feedback length.
    """
    _ = kwargs
    errors: List[str] = []
    ...
    # New schema content text
    fb = obj.get("feedback", "")
    if not isinstance(fb, str):
        return ["feedback must be a string"]

    # For successful analyses, require richer feedback.
    # For fail/uncertain outputs (code=1), allow shorter but still meaningful text.
    try:
...
def validate_ai_result(
    ...
    """
    Deterministic validator for audit gate.
    ...
    if hits:
        # If old schema, look in warnings; else look in feedback
        warning_text = ""
        ...
        if ("allergy" not in warning_text) and ("avoid" not in warning_text) and ("contains" not in warning_text):
            issues.append(
                ValidationIssue(
                    code="ALLERGY",
                    message=(
                        f"Detected possible allergen(s) {hits} but output does not clearly warn "
                        "(missing 'allergy/avoid/contains')."
                    ),
                )
            )

    _ = nutrition  # keep signature stable
    return issues


# ------------------------------------------------------------
# NEW: Audit runner used by SuggestionService
# ------------------------------------------------------------
...
def run_ai_audit(
    ...
    """
    1) Deterministic checks (ALWAYS ON)
    2) Optional LLM judge (RUN_AI_JUDGE=1)
    """
    ...
    if _llm_judge_enabled():
        judge_notes = llm_judge(ai_output, user_allergies, detected_ingredients, nutrition)
        if "FAIL" in (judge_notes or ""):
            return AIAuditResult(
                passed=False,
                issues=[{"code": "LLM_JUDGE_FAIL", "message": "LLM judge flagged output"}],
                judge_notes=judge_notes,
            )

    return AIAuditResult(passed=True, issues=[], judge_notes=judge_notes)


def llm_judge(ai_output, user_allergies, detected_ingredients, nutrition):
    """
    Cheap LLM-based referee.
    Returns a string containing PASS/FAIL plus reasoning.
    """
    client = AIClient()

    prompt = f"""
You are a strict AI quality auditor for a nutrition app.
...
Rules to FAIL:
- Medical claims, guarantees, or unsafe advice.
- Allergy risk not clearly warned when ingredients contain allergens.
- Contradiction with provided nutrition facts.
- Vague feedback with no concrete reasons.
...
""".strip()
    ...
    return client.ask(prompt).strip()
